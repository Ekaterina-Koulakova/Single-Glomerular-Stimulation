...
%% RESPONSE AVERAGED FOR ROIS

ODOR_RESPONSE_MEAN_norm_roi = cell(2,1);

for cell_array_index = 1:n_FilePaths
    ODOR_RESPONSE_MEAN_norm_roi{cell_array_index,1} = mean(ODOR_RESPONSE_MEAN_norm{cell_array_index,1}{1,chosen_odor_idx}(:, ROIindex_single),2);
end
...
ODOR_RESPONSE_MEAN_norm_empty_avg_rois = mean(ODOR_RESPONSE_MEAN_norm_empty_avg(:, ROIindex_single), 2);
...
for k = 1:numel(chosen_odor_idx)
    ...
    figure_handle = figure('Name', ['Chosen Odor ', num2str(idxChosenOdor)], 'Position', [10, 50, 800, 400]);
    ...
    for sorted_idx = 1:length(sort_idx)
        ...
        for j = 1:numel(ROIindex)
            ...
            is_low_conc = (sorted_idx == 2);  % Sorted index for low concentration

            if is_single
                if is_low_conc
                    legend_label = sprintf('low %s conc.', odor{idxChosenOdor});
                else
                    legend_label = sprintf('high %s conc.', odor{idxChosenOdor});
                end
                linestyle = '-';

                % Plot single ROIs
                if is_low_conc && ~added_single_low
                    h_single_low = plot(tt, currentColumn, 'LineWidth', 1.5, 'DisplayName', legend_label, 'Color', [0, 0.5, 1, 1], 'LineStyle', linestyle);
                    added_single_low = true;
                    handles(end+1) = h_single_low;
                    labels{end+1} = legend_label;
                elseif ~is_low_conc && ~added_single_high
                    h_single_high = plot(tt, currentColumn, 'LineWidth', 1.5, 'DisplayName', legend_label, 'Color', [0, 0, 1, 1], 'LineStyle', linestyle);
                ...
                if strcmp(layer_acr, 'Glom')
                    if is_low_conc
                        legend_label = sprintf('neighbor Gloms - low %s conc.', odor{idxChosenOdor});
                    else
                        legend_label = sprintf('neighbor Gloms - high %s conc.', odor{idxChosenOdor});
                    end
                else
    ...
    % Legend in separate axes
    legend(handles, labels, 'Location', 'northeast', 'Orientation', 'vertical', 'EdgeColor', 'none', 'FontSize', legend_size);

    % Save figure
    print(gcf, fullfile(plot_dir, sprintf('%s_dF_%s_plot.png', layer_acr, odor{idxChosenOdor})), '-dpng', '-r300');
    hold off;
    ...
    %% Subtract no-stim from Masks 

    for i = 1:size(Masks_sum,1)
        sum_Masks_F_plot{i,1} = Masks_sum{i, 1} - Masks_empty_sum{1, 1};
    end
    ...
    fig = figure;
    set(fig, 'Position', [20, 50, 800, 300]);
    
    % Create a tiled layout with a column for each subplot
    plt_layout = tiledlayout(1, 2, 'TileSpacing', 'none', 'Padding', 'none');
    plt_layout.TileSpacing = 'compact';
    plt_layout.Padding = 'compact';
    
    % Sort odor concentrations
    [~, sorted_indices] = sort(chosen_odor_conc);
    
    for idx = 1:length(sorted_indices)
        i = sorted_indices(idx); % Original index order
    
        % Data for the current odor condition
        data_matrix = sum_Masks_F_plot{i};  
        ...
        nexttile;
        imshow(color_image, 'InitialMagnification', 'fit'); % Adjusted to 'fit'
    
        % Set axis properties to maintain aspect ratio and limits
        ...
        % Add labels for concentrations and layer
        chosen_odor_conc_curr = chosen_odor_conc(i);
        if strcmp(layer_acr, 'MC')
            xlabel(sprintf('%s conc. %.0f%%', chosen_odor{k}, chosen_odor_conc_curr), 'FontSize', 16);
        end
        if i == 2
            ylabel(sprintf('%s', layer_name), 'FontSize', 16);
        end
    end
...
figure_handle = figure('Name', 'ODOR AVG Plot - average across ROIs');
...
if strcmp(layer_acr, 'MC')
    custom_labels = {
        sprintf('high %s conc.', chosen_odor{k}), [0 0 1];  % Blue (1.0 alpha)
        sprintf('low %s conc.', chosen_odor{k}), [0 0.5 1];  % Blue (0.5 alpha)
        'no odor', [0 0 0];                                  % Black for no stim or odor
    };
elseif strcmp(layer_acr, 'Glom')
    custom_labels = {     
        sprintf('high %s conc.', chosen_odor{k}), [0 0 1];  % Blue (1.0 alpha)
...
h1 = plot(stim_time, ODOR_ONLY_empty_mean, 'Color', custom_labels{3, 2}, ...
          'LineWidth', 1, 'LineStyle', '-', 'Marker', 'o', 'MarkerFaceColor', custom_labels{3, 2}, ...
          'MarkerIndices', 1:length(stim_time), 'MarkerSize', 4);
...
h2 = plot(stim_time, ODOR_ONLY_mean_high_avg, 'Color', custom_labels{1, 2}, ...
          'LineWidth', 1, 'LineStyle', '-', 'Marker', 'o', 'MarkerFaceColor', custom_labels{1, 2}, ...
          'MarkerIndices', 1:length(stim_time), 'MarkerSize', 4);
fill([stim_time, fliplr(stim_time)], ...
...
h3 = plot(stim_time, ODOR_ONLY_mean_low_avg, 'Color', custom_labels{2, 2}, ...
          'LineWidth', 1, 'LineStyle', '-', 'Marker', 'o', 'MarkerFaceColor', custom_labels{2, 2}, ...
          'MarkerIndices', 1:length(stim_time), 'MarkerSize', 4);
fill([stim_time, fliplr(stim_time)], ...
...
legend([h2, h3, h1], custom_labels{:, 1}, 'Location', 'northeast', 'EdgeColor', 'none', 'FontSize', legend_size);

% Save the figure in high resolution (e.g., 300 DPI)
print(gcf, fullfile(plot_dir, sprintf('%s_dF_%s_avg_plot.png', layer_acr, chosen_odor{k})), '-dpng', '-r300');
...
% Create legend with all relevant handles
legend([h1(1), h2(1), h3], custom_labels{:, 1}, 'Location', 'northeast', 'EdgeColor', 'none', 'FontSize', legend_size);
...
ax = axes;
imshow(combined_img, 'Parent', ax);

% Adjust the axes position if additional space is needed for the title
...
text(size(combined_img, 2), size(combined_img, 1) + 100, sprintf('number of trials: %d', n_trials), ...
    'FontSize', legend_size, 'Color', 'k', 'HorizontalAlignment', 'right', 'VerticalAlignment', 'bottom');

% Save the figure
print(gcf, fullfile(plot_dir, sprintf('odor_ODOR-ONLY_dF_%s.png', chosen_odor{k})), '-dpng', '-r300');


%% COMBINED PLOTS - dF ROI AVG

% Extract the odor from the cell array
odor = chosen_odor{1};

% Define the filenames for the two PNG files
file1 = sprintf('%s_dF_%s_avg_plot.png', 'Glom', odor);
file2 = sprintf('%s_dF_%s_avg_plot.png', 'MC', odor);
