...
for i = 1:length(ROIindex);
    ...
    if strcmp(layer_acr, 'Glom')
        legend('Location', 'northeastoutside', 'Orientation', 'vertical', 'EdgeColor', 'none', 'FontSize', legend_size);
    end
...
for i = 1:n_latencies
    sum_Masks_F_plot{i} = sum_Masks_F{1, i} - sum_Masks_F{2, i};
end
...
plt_layout = tiledlayout(1, n_latencies, 'TileSpacing', 'none', 'Padding', 'none');
...
for k = 1:n_latencies
    ...
    data_matrix = sum_Masks_F_plot{1, k};
...
if strcmp(layer_acr, 'MC')
    custom_labels = {
        'stim', [1 0 0];  % Red for stim
        'no stim', [0 0 0];    % Black for no stim or odor
    };
elseif strcmp(layer_acr, 'Glom')
    custom_labels = {
        'stim', [1 0 0];  % Red for stim
...
h1 = plot(stim_time, STIM_ONLY_no_stim_mean, 'Color', custom_labels{2, 2}, ...
          ...
          'MarkerFaceColor', custom_labels{2, 2}, 'MarkerSize', 4, ...
          'MarkerIndices', 1:length(stim_time));
...
for i = 1:size(STIM_ONLY_numeric, 1)
    ...
    h2(i) = plot(stim_time, STIM_ONLY_numeric(i, :), 'Color', custom_labels{1, 2}, ...
                 'LineWidth', 1, 'LineStyle', '-', 'Marker', 'o', ...
                 'MarkerFaceColor', custom_labels{1, 2}, 'MarkerSize', 4, ...
                 'MarkerIndices', 1:length(stim_time));
...
legend([h2(1) h1], custom_labels{:, 1}, 'Location', 'northeast', 'EdgeColor', 'none', 'FontSize', legend_size);
